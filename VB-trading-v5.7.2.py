...
# 세팅
coin_list = ['KRW-BTC', 'KRW-ETH', 'KRW-PLA']
buy_check = {'KRW-BTC': 0, 'KRW-ETH': 0, 'KRW-PLA': 1}
    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(minutes=1):
            if update_check == 0:
                ...
                investing_amts = []
                for coin in coin_list:
                    investing_rate = get_investing_rate(coin, 99)
            ...
            for ind, coin in enumerate(coin_list):
                ...
                if (buy_check[coin] == 0) and ((coin == 'KRW-BTC') or (coin == 'KRW-ETH')):
                    if (target_prices[ind] < current_price) and (get_ma5(coin) < target_prices[ind]):
                        if ind == 0:
                            upbit.buy_market_order(coin, investing_amts[ind]*1.5)
                        if ind == 1:
                ...
                if buy_check[coin] == 1:
                    if coin == 'KRW-BTC':
                        if 0.95 * target_prices[ind] > current_price:
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                    ...
                    if coin == 'KRW-ETH':
                        if 0.925 * target_prices[ind] > current_price:
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                        time.sleep(0.1)
                    ...
                    if coin == 'KRW-PLA':
                        if (0.85 * 1395 > current_price) or (1.15 * 1395 < current_price):
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                            buy_check[coin] = 0
