...
# 세팅
coin_list = ['KRW-BTC', 'KRW-ETH', 'KRW-UPP', 'KRW-OMG', 'KRW-GRS']
investing_limit = 17600000
buy_check = {'KRW-BTC': 0}
high_prices = {'KRW-ETH': [5350000]}
...
#             print(now, buy_check, target_prices, investing_amts, high_prices)
            ...
            for ind, coin in enumerate(coin_list):
                ...
                if buy_check[coin] == 1:
                    ...
                    if coin == 'KRW-UPP':
                        if (0.9 * 234 > current_price) or (1.1 * 234 < current_price):
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                            high_prices[coin] = [0]
                    ...
                    if coin == 'KRW-GRS':
                        if (0.9 * 1150 > current_price) or (1.1 * 1150 < current_price):
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                            high_prices[coin] = [0]
