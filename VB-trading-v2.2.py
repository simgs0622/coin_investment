...
#                         print(coin, ' 전략2로 매수했음ㅋ_ㅋ')
            ...
            target_prices = []
            for ind, coin in enumerate(coin_list):
                target_price = get_target_price(coin, 0.6, base)
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=59):
            for ind, coin in enumerate(coin_list):
                ...
                # 상승장의 정도를 0, 0.5, 1로 판단 for 전략 1
                try:
                    if (get_ma5(coin, base) < current_price) or (get_ma10(coin, base) < current_price):
                        investing_rate2 = 0.5
                    else:
                        investing_rate2 = 0
                    time.sleep(0.1)
                    if (get_ma5(coin, base) < current_price) and (get_ma10(coin, base) < current_price):
                        investing_rate2 = 1
                except:
                    if (get_ma5(coin, base) < current_price) or (get_ma9(coin, base) < current_price):
                        investing_rate2 = 0.5
                    else:
                        investing_rate2 = 0
                    time.sleep(0.1)
                    if (get_ma5(coin, base) < current_price) and (get_ma9(coin, base) < current_price):
                        investing_rate2 = 1
