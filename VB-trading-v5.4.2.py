...
# 세팅
coin_list = ['KRW-BTC', 'KRW-ETH', 'KRW-VET', 'KRW-OMG', 'KRW-ETC', 'KRW-DAWN']
investing_limit = 16800000
buy_check = {'KRW-BTC': 0}
...
#             print(now, buy_check, target_prices, investing_amts, high_prices)
            ...
            for ind, coin in enumerate(coin_list):
                ...
                if buy_check[coin] == 1:
                    ...
                    if coin == 'KRW-VET':
                        if (0.9 * 141 > current_price) or (1.15 * 141 < current_price):
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                            high_prices[coin] = [0]
                    ...
                    if coin == 'KRW-OMG':
                        if (0.8 * 17047 > current_price) or (1.3 * 17047 < current_price):
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                            high_prices[coin] = [0]
                    ...
                    if coin == 'KRW-ETC':
                        if (0.9 * 66720 > current_price) or (1.15 * 66720 < current_price):
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                            high_prices[coin] = [0]
                    ...
                    if coin == 'KRW-DAWN':
                        if (0.9 * 4102 > current_price) or (1.15 * 4102 < current_price):
                            units = get_balance(coin_s)
                            if units > 0.00000001:
                                upbit.sell_market_order(coin, units)
                            high_prices[coin] = [0]
