...
df_result_sub = df_result[df_result['noise'] < 0.55]
if df_result_sub.shape[0] < 3:
    coin_list = ['KRW-BTC', 'KRW-LTC', 'KRW-ETC']
    coin_shortlist = ['BTC', 'LTC', 'ETC']
    noise_list = [0.55, 0.55, 0.55]
else:
    df_result_sub = df_result_sub.sort_values(by=['vola'], ascending=False)
    df_result_sub = df_result_sub.reset_index(drop='index')
    df_result_sub = df_result_sub[:3]
...
def get_target_price(ticker, k, base, jap_yn):
    """변동성 돌파 전략으로 매수 목표가 조회"""
    df = pyupbit.get_daily_ohlcv_from_base(ticker, base=base)
    ran1 = df.iloc[-2]['high'] - df.iloc[-2]['low']  # 전날 변동폭
    ran2 = df.iloc[-3]['high'] - df.iloc[-3]['low']
    ran = (ran1 + ran2) / 2
    ran *= (2/3)  # 12시간 변동폭 추정치
    if jap_yn:
        target_price = df.iloc[-2]['close'] + (ran) * k * 1.4
    else:
        target_price = df.iloc[-2]['close'] + (ran) * k * 0.9
    return target_price

def get_investing_rate(ticker, tv, base):
    """변동성 통제를 위한 투자비율 산출"""
    df = pyupbit.get_daily_ohlcv_from_base(ticker, base=base)
    vol1 = ((df.iloc[-2]['high'] - df.iloc[-2]['low']) / df.iloc[-2]['open']) * 100  # 전날 변동률
    vol2 = ((df.iloc[-3]['high'] - df.iloc[-3]['low']) / df.iloc[-3]['open']) * 100
    vol = (vol1 + vol2) / 2
    vol *= (2/3)
    if vol < tv:
        investing_rate = 1
...
for ind, coin in enumerate(coin_list):
    if ind < 3:
        target_price = get_target_price(coin, noise_list[ind], 20, True)
    else:
        target_price = get_target_price(coin, noise_list[ind], 20, False)
...
#         print(coin, ' 투자금액: ', investing_price)
        ...
        if start_time < now < check_time:
            ...
            if update_yn == 0:
                ...
                noise_list = []
                for coin in krw_tickers:
                    df = pyupbit.get_daily_ohlcv_from_base(coin, base)
                    df['vola'] = (df['high'] - df['low']) / df['open'] * 100
                    df['noise'] = 1 - (abs(df['close'] - df['open']) / (df['high'] - df['low']))
...
#             print('전략2 투자금액 판단중', start_time, now)
            investing_prices2 = []
            for ind, coin in enumerate(coin_list):
                investing_rate = get_investing_rate(coin, 4, base)
...
#                     investing_price = invest_limit * investing_rate * 0.9995
                    investing_price = invest_limit * investing_rate * 0.9995 * 0
...
#                         print(coin, ' 전략2로 매수했음ㅋ_ㅋ')
            ...
            for ind, coin in enumerate(coin_list):
                if ind < 3:
                    target_price = get_target_price(coin, noise_list[ind], base, True)
                else:
                    target_price = get_target_price(coin, noise_list[ind], base, False)
                target_prices.append(target_price)
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=59):
            for ind, coin in enumerate(coin_list):
                ...
                if ind < 3:
                    investing_rate = get_investing_rate(coin, 10, base)
                else:
