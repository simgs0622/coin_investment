...
# 세팅
coin_list = ['KRW-ETH', 'KRW-BTC', 'KRW-MED']
coin_shortlist = ['ETH', 'BTC', 'MED']
BTC_limit = 20000000
...
if my_balance > BTC_limit:
    investing_limit = BTC_limit
else:
    investing_limit = my_balance * 0.99
...
#             print(now, buy_check, target_prices, investing_amts, high_prices)
            ...
            for ind, coin in enumerate(coin_list):
                ...
                if (buy_check[coin] == 1) and (coin != 'KRW-BTC'):
                    if (get_ma60(coin) > current_price) or (0.7 * max(high_prices[coin]) > current_price):
