...
tmp_close = 0
pred_val = 0
new_model = load_model('ST_prediction_l32_r10_095_0.560.h5', compile=False)
    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(minutes=1):
            if update_check == 0:
                ...
                investing_limit = my_balance * 0.25
            ...
            for ind, coin in enumerate(coin_list):
                ...
                if (st_check[coin] == 0) and (coin == 'KRW-ETH'):
                    if (now.minute % 5 == 4) and (now.second >= 58):
                        df1 = pyupbit.get_ohlcv('KRW-BTC', interval='minute5', count=25)
                        df2 = pyupbit.get_ohlcv('KRW-ETH', interval='minute5', count=25)
                        ...
                        df_sub['open_x'] = df_sub['open_x'] / d_x
                        df_sub['high_x'] = df_sub['high_x'] / d_x
                        df_sub['low_x'] = df_sub['low_x'] / d_x
                        df_sub['close_x'] = df_sub['close_x'] / d_x
                        df_sub['volume_x'] = df_sub['volume_x'] / d_vx
                        df_sub['open_y'] = df_sub['open_y'] / d_y
                        df_sub['high_y'] = df_sub['high_y'] / d_y
                        df_sub['low_y'] = df_sub['low_y'] / d_y
                        df_sub['close_y'] = df_sub['close_y'] / d_y
                        df_sub['volume_y'] = df_sub['volume_y'] / d_vy

                        xtest = np.array(df_sub)
                        pred = new_model.predict(xtest.reshape(1,25,10))
                        pred_val = pred[0][0]
                ...
                if st_check[coin] == 1:
                    if (0.99 * tmp_close > current_price) or (1.01 * tmp_close < current_price):
                        units = get_balance(coin_s)
                        if units > 0.00000001:
                            if buy_check[coin] == 1:
